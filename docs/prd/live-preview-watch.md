...
引入**画面监看**：Web 点选已占用且在线设备即开启独立监看会话；媒体复用腾讯云 TRTC 与连线共摄旁路（ADR-0002/0003），设备只推视频、平台只拉视频，不对讲、不打断 AI。设备侧红灯常亮（与指挥连线同灯效），无语音提示。监看可同房升级为指挥连线（升麦/对讲并打断 AI，仍无 TTS）；也可不经监看直接发起连线。同一设备同时仅一路监看或一路连线。结束：换设备、停止监看、或 Web 关页/断连；首期无空闲超时。GB28181 与 JPEG `/frame` 不承接本能力。
...
7. As a 指挥中心座席, I want 关闭或刷新浏览器页时监看自动结束, so that 设备不会一直红灯推流。
...
12. As an 执勤员, I want 被画面监看时仅红灯常亮、无语音提示, so that 我知道画面通道被占用但不被 TTS 打断作业。
...
15. As an 执勤员, I want 直接指挥连线与监看升级均无来电语音播报, so that 提示规则统一（仅靠红灯与业务行为感知）。
...
18. As an 执勤员, I want 监看结束后红灯恢复为当时应有模式（如录像闪烁）, so that 指示灯不粘滞。
...
29. As an Android 设备, I want 收到监看开始后自动进房、共摄推视频、红灯常亮、不启连线对讲、不打断 AI, so that 行为符合画面监看定义。
30. As an Android 设备, I want 收到升级为指挥连线后开启对讲策略并打断 AI, so that 同房升级完整。
31. As an Android 设备, I want 收到监看结束后退房并清理灯效/推流, so that 与结束连线对称。
...
- 指挥连线 PRD 中「来电语音提示」由本 PRD 修正为：**直接连线与监看升级均无 TTS**；红灯常亮保留；升级/直接连线仍打断 AI。
...
点选已占用在线设备
  ...
  → MQTT/HTTP 监看开始 → 设备共摄进房推视频、红灯常亮、不打断 AI
  → Web 进房只拉视频
...
点选 → 设备进房只推视频 → Web 见画面 → 升级为指挥连线或结束 → 状态与红灯清理。假 TRTC / 假 MQTT。
