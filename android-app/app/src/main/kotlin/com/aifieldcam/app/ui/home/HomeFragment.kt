...
class HomeFragment : Fragment(), SessionManager.StatusListener {
    ...
    private val takePictureLauncher = registerForActivityResult(
        ...
        Toast.makeText(requireContext(), "照片已保存，可在相册查看", Toast.LENGTH_SHORT).show()
    }
    ...
    private fun refreshUi() {
        ...
        binding.tvPhoneHint.text = if (connected) {
            "已连接 BLE 相机，拍照/录像由外接相机执行"
        } else {
            "未连接 BLE 时，可使用手机相机拍照/录像并保存到相册"
        }
        binding.btnConnect.isEnabled = ble.canStartConnect()
        binding.btnDisconnect.isEnabled = connected
        binding.btnCapture.isEnabled = connected || CameraPermissionHelper.hasCamera(requireContext())
        binding.btnStartRec.isEnabled = connected || CameraPermissionHelper.hasCamera(requireContext())
        binding.btnStopRec.isEnabled = connected
